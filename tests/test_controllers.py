...
class TestControllers(unittest.TestCase):
    def setUp(self):
        # Mock user setup
        self.mock_user = Mock(
            username="test_user", role_id="Commercial", email="test@example.com"
        )
        ...
        # Mock event setup
        self.mock_event = Mock(id=1, contract_id=1, support_contact_id=None)
    ...
    def test_has_permission_granted(self):
        with patch("main.models.User.get_by_username", return_value=self.mock_user):
            with patch("main.models.Role.get_by_name", return_value=self.mock_role):
                with patch(
                    "main.models.Permission.get_permissions_by_role",
                    return_value=self.mock_permissions,
                ):
                    result = has_permission("test_user", "client", "create")
            ...
            with patch("main.models.Role.get_by_name", return_value=commercial_role):
                with patch(
                    ...
                    result = has_permission(
                        "sales", "event", "create", resource_owner_username="sales"
                    )
                    self.assertTrue(result)
    ...
    def test_delete_event_success(self):
        with patch("main.models.Event.get_by_id", return_value=self.mock_event):
            with patch(
                "main.models.Contract.get_by_id", return_value=self.mock_contract
            ):
                with patch(
                    "main.models.Client.get_by_email", return_value=self.mock_client
                ):
    ...
    def test_get_all_events_success(self):
        ...
        with patch("main.models.User.get_by_username", return_value=self.mock_user):
            with patch("main.models.Role.get_by_name", return_value=self.mock_role):
                with patch(
                    "main.models.Database.connect",
                    return_value=self.create_db_mock(mock_data),
                ):
            ...
            with patch("main.models.Role.get_by_name", return_value=self.mock_role):
                with patch(
                    ...
                    with patch(
                        ...
                        with patch.object(
                            self.mock_client, "update", return_value=True
                        ):
                            result = update_client(
                                "test_user", "client@example.com", first_name="Jane"
                            )
                            self.assertIn("updated successfully", result)
                    ...
                    with patch(
                        ...
                        with patch("main.models.Contract.create", return_value=True):
                            result = create_contract(
                                "test_user", "client@example.com", 1000, 1000, "Pending"
                            )
                            self.assertIn("created successfully", result)
        ...
        with patch("main.models.User.get_by_username", return_value=self.mock_user):
            with patch("main.models.Role.get_by_name", return_value=self.mock_role):
                with patch(
                    ...
                    with patch(
                        "main.models.Contract.get_by_id",
                        return_value=self.mock_contract,
                    ):
                        with patch.object(
                            self.mock_contract, "update", return_value=True
                        ):
                            result = update_contract(
                                "test_user", 1, 2000, 1500, "Active"
                            )
                            self.assertIn("updated successfully", result)
            ...
            with patch("main.models.Role.get_by_name", return_value=self.mock_role):
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                            "main.models.Client.get_by_email",
    ...
    def test_get_all_clients_success(self):
        ...
        with patch(
            "main.models.Database.connect", return_value=self.create_db_mock(mock_data)
        ):
        ...
        with patch("main.models.User.get_by_username", return_value=self.mock_user):
            with patch("main.models.Role.get_by_name", return_value=self.mock_role):
                with patch(
                    ...
                    with patch(
                        "main.models.Event.get_by_id", return_value=self.mock_event
                    ):
                        with patch(
                            "main.controllers.has_permission", return_value=True
                        ):
                            with patch.object(
                                self.mock_event, "update", return_value=True
                            ):
                                result = assign_support_to_event(
                                    "admin_user", 1, "support_user"
                                )
    ...
    def test_permission_denied_for_create_client(self):
        with patch("main.models.User.get_by_username", return_value=self.mock_user):
            with patch("main.models.Role.get_by_name", return_value=self.mock_role):
                with patch(
                    "main.models.Permission.get_permissions_by_role", return_value=[]
            ...
            with patch("main.models.Role.get_by_name", return_value=self.mock_role):
                with patch(
                    ...
                    with patch(
                        ...
                        self.assertIn(
                            "CHECK constraint failed: status IN ('Signed', 'Not Signed')",
                            result,
                        )
        ...
        with patch("main.models.User.get_by_username", return_value=self.mock_user):
            with patch("main.models.Role.get_by_name", return_value=self.mock_role):
                with patch(
                    ...
                    with patch(
                        "main.models.Contract.get_by_id", return_value=None
                    ):  # Contract not found
    ...
    def test_client_not_found_for_update(self):
        with patch("main.models.Client.get_by_email", return_value=None):
            result = update_client(
                "test_user", "nonexistent@example.com", first_name="NewName"
            )
    ...
    def test_database_error_handling(self):
        ...
        with patch(
            "main.models.Database.connect", side_effect=sqlite3.Error("Database error")
        ):
        ...
        with patch("main.models.User.get_by_username", return_value=self.mock_user):
            with patch("main.models.Role.get_by_name", return_value=self.mock_role):
                with patch(
                    ...
                    result = update_user(
                        "test_user", "existing_user", email="updated_email@example.com"
                    )
                    self.assertEqual(result, "Permission denied.")
