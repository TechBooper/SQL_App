...
class TestDatabaseInitialization(unittest.TestCase):
    ...
    def test_get_role_id(self):
        from main.database import create_connection, create_tables_and_triggers, get_role_id
    ...
    def test_create_user(self):
        from main.database import create_connection, create_tables_and_triggers, create_user
    ...
    @patch('builtins.input', side_effect=["admin", "admin@example.com"])
    @patch('getpass.getpass', side_effect=["weakpass", "weakpass", "StrongP4ss!", "StrongP4ss!"])
    def test_initialize_database_weak_password(self, mock_getpass, mock_input):
        ...
        self.assertIsNotNone(user)
        self.assertTrue(bcrypt.checkpw(b"StrongP4ss!", user["password_hash"].encode('utf-8')))
        conn.close()

    @patch('builtins.input', side_effect=["admin", "admin@example.com"])
    @patch('getpass.getpass', side_effect=["StrongP4ss!", "MismatchP4ss!", "StrongP4ss!", "StrongP4ss!"])
