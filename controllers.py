...
def create_client(user_id, first_name, last_name, email, phone, company_name):
    if not first_name or not last_name or not email or not phone or not company_name:
...
def create_user(admin_user_id, username, password, role_id, email):
    ...
    try:
        ...
        if existing_user:
            logging.warning(
                f"Attempt to create duplicate username '{username}' by admin user ID {admin_user_id}."
            )
