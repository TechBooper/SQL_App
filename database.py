...
def initialize_database():
    if not os.path.exists('app.db'):
        with sqlite3.connect('app.db') as conn:
            ...
            ''')
            conn.commit()
            logging.info("Database initialized with tables 'roles', 'users', and 'permissions'.")
    ...
    try:
        with sqlite3.connect('app.db') as conn:
            ...
            if user:
                ...
                stored_password_hash = base64.b64decode(user['password_hash'].encode('utf-8'))
                ...
                if bcrypt.checkpw(password_bytes, stored_password_hash):
                    ...
                    logging.warning(f"Failed authentication attempt for username: {username}. Incorrect password.")
                    return None
            else:
                logging.warning(f"Failed authentication attempt for username: {username}. User not found.")
                return None
    except sqlite3.Error as e:
